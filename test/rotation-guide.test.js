...
test('Rotation Guide filters sourced priorities through the learned spellbook', async () => {
  ...
  for (const profile of [
    'Guardian:Gladiator', 'Necromancer:Animation', 'Cultist:Heretic', 'Sun Cleric:Valkyrie',
    'Runemaster:Engravement', 'Runemaster:Glyphic', 'Runemaster:Riftblade'
  ]) {
  ...
  assert.match(data, /"Primordial Blast", "Runic Brand", "Runeblade"/);
  assert.match(data, /"Elemental Burst", "Primordial Blast", "Glyphic Ruin"/);
...
test('Rotation Guide keeps preparation separate and never automates gameplay', async () => {
  ...
  for (const required of [
    ...
    'buttons.learn', 'buttons.rotation', 'buttons.situations', 'buttons.progression', 'buttons.updates'
...
test('Rotation Guide uses a warmer navigable card layout', async () => {
  ...
  for (const required of [
    ...
    'row.accent:SetVertexColor', 'COMPRENDRE', 'ROTATION', 'SITUATIONS', 'PROGRESSION', 'ACTUS', 'SOURCES',
    'ACTUALISER MAINTENANT', 'header:SetFrameLevel(guideFrame:GetFrameLevel() + 1)',
    'guideFrame:SetHeight(748)', 'GameTooltip.SetSpellBookItem'
...
test('Rotation Guide refreshes after level, spell and CoA talent changes', async () => {
  const lua = await luaPromise;
  for (const required of [
    'CharacterFingerprint', 'SpellbookSlotCount', 'lastCharacterFingerprint',
    'PLAYER_LEVEL_UP', 'SPELLS_CHANGED', 'LEARNED_SPELL_IN_TAB', 'CHARACTER_POINTS_CHANGED',
    'CHARACTER_ADVANCEMENT_UPDATE_ENTRIES_RESULT', 'ASCENSION_CA_SPECIALIZATION_ACTIVE_ID_CHANGED',
    'gain de niveau', 'spellbook modifie', 'talents ou specialisation modifies',
    'remainingPasses', 'evolution detectee automatiquement', 'CoARotationGuideDB.characters',
    'CoALootDeciderAPI.GetAdaptiveBuild', 'activeTalentSource = "arbres CoA"'
  ]) assert.ok(lua.includes(required), `automatic refresh is missing ${required}`);
  assert.match(lua, /if not spellbook\[key\] then table\.insert\(orderKeys, key\) end[\s\S]+index >= \(spellbook\[key\]\.index or 0\)/,
    'spellbook ranks must be deduplicated while keeping the newest rank');
});

test('Rotation Guide embeds a level-aware offline progression path and live loot profile', async () => {
  const [lua, progression] = await Promise.all([luaPromise, progressionPromise]);
  assert.doesNotThrow(() => luaparse.parse(progression, { luaVersion: '5.1', comments: false, locations: true }));
  for (const required of [
    'ProgressionBand', 'ProgressionCards', 'LootProfileSummary', 'viewMode == "PROGRESSION"',
    'Mythiques, caches et Edrim Skysong', 'CoALootDeciderAPI.GetProfile', 'CoALootDeciderAPI.GetDisplayStats'
  ]) assert.ok(lua.includes(required), `progression UI is missing ${required}`);
  for (const required of [
    'Ascension - Mythic+ Releases', 'Edrim Skysong', 'Mythic Coins',
    'maximum = 9', 'maximum = 19', 'maximum = 39', 'maximum = 49', 'maximum = 59', 'maximum = 999'
  ]) assert.ok(progression.includes(required), `progression bank is missing ${required}`);
});
