...
const addons = ['CoACombatAssistant', 'CoAUIManager', 'CoARotationGuide', 'CoADungeonNavigator', 'CoAEssentialAssistant', 'CoAStormbringerHelper', 'CoAPrimalistHelper'];
...
for (const name of addons) {
  test(`${name} targets Ascension 3.3.5 and parses as Lua 5.1`, async () => {
    ...
    for (const api of forbiddenRetailApis) {
      ...
      // talent-tab fallback.
      if ((name === 'CoARotationGuide' || name === 'CoAEssentialAssistant' || name === 'CoAStormbringerHelper' || name === 'CoAPrimalistHelper') && api === 'GetSpecialization') continue;
...
test('UI Manager provides persistent movers and never applies frames during combat', async () => {
  ...
  for (const frame of [
    ...
    'CoAEssentialAssistantHUD', 'CoAEssentialResourceHUD', 'CoAEssentialTargetHUD',
    'CoAEssentialCoverageHUD', 'CoAEssentialAssistantSettings'
...
test('UI Manager minimap button remains the hub for shared tools but leaves Loot Decider standalone', async () => {
  const lua = await readFile('addons/CoAUIManager/CoAUIManager.lua', 'utf8');
  for (const required of [
    'Centre CoA', 'OUTILS COA', 'Heretic', 'Rotations', 'Messages', 'Storm', 'Primalist', 'Essentiel',
    'CoAHereticHelperAPI:Toggle()',
    'CoAMessageCenter:Toggle()', 'CoARotationGuideAPI:Toggle()', 'CoADungeonNavigatorAPI:Toggle()', 'CoAStormbringerHelperAPI:Toggle()', 'CoAPrimalistHelperAPI:Toggle()', 'CoAEssentialAssistantAPI:Toggle()', 'SetHubManaged(true)',
