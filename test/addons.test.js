...
import { AddonManager, WARMANE_ADDONS, compareAddonVersions, parseToc } from '../src/core/addons.js';
...
test('manager scans Ascension addons and installs, backs up, restores and updates CoA addons', async () => {
  ...
  try {
    ...
    assert.equal(inventory.detectionSource, 'warmane');
...
test('a manually selected AddOns path is remembered when standard detection fails', async () => {
  ...
  try {
    ...
    assert.deepEqual(await second.detectDirectory(), { directory: selected, exists: true, source: 'saved', gameFlavor: 'warmane' });
...
test('a manually selected AddOns path overrides another valid Ascension installation', async () => {
  ...
  try {
    ...
    assert.deepEqual(await manager.detectDirectory(), { directory: canonical, exists: true, source: 'warmane', gameFlavor: 'warmane' });
    await manager.setDirectory(selected);
    assert.deepEqual(await manager.detectDirectory(), { directory: selected, exists: true, source: 'saved', gameFlavor: 'warmane' });
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
let realWarmaneExists = false;
try { realWarmaneExists = (await stat(WARMANE_ADDONS)).isDirectory(); } catch { /* CI does not have Warmane. */ }
test('scanner reads the conventional Warmane installation when present', { skip: !realWarmaneExists }, async () => {
  const manager = new AddonManager({ dataDir: path.join(tmpdir(), 'coa-real-scan'), manifestUrl: 'http://invalid.local/manifest.json' });
  const addons = await manager.scan(WARMANE_ADDONS);
