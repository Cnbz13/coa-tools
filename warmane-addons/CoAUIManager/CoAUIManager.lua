...
local DEFAULT_FRAMES = {
    ...
    "CoAUIManagerPanel", "CoALootDeciderBanner", "CoALootAdvisorWindow"
}
...
local FRAME_LABELS = {
    ...
    CoAUIManagerPanel = "UI Manager Warmane",
...
local function Chat(message)
    if DEFAULT_CHAT_FRAME then
        DEFAULT_CHAT_FRAME:AddMessage("|cffffcc66UI Manager Warmane:|r " .. tostring(message))
    end
end
...
local function EnsureDatabase()
    ...
    if type(CoAUIManagerDB.muteErrorSpeech) ~= "boolean" then CoAUIManagerDB.muteErrorSpeech = true end
    if type(CoAUIManagerDB.hideErrorMessages) ~= "boolean" then CoAUIManagerDB.hideErrorMessages = true end
...
panelTitle:SetText("UI Manager • Warmane")
...
local comfortLabel = panel:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
comfortLabel:SetPoint("TOPLEFT", panel, "TOPLEFT", 18, -98)
comfortLabel:SetText("CONFORT DE JEU")
comfortLabel:SetTextColor(0.35, 0.82, 1.00)

local muteSoundButton = CreateFrame("Button", nil, panel, "UIPanelButtonTemplate")
muteSoundButton:SetWidth(158)
muteSoundButton:SetHeight(25)
muteSoundButton:SetPoint("TOPLEFT", panel, "TOPLEFT", 18, -118)

local hideErrorsButton = CreateFrame("Button", nil, panel, "UIPanelButtonTemplate")
hideErrorsButton:SetWidth(158)
hideErrorsButton:SetHeight(25)
hideErrorsButton:SetPoint("LEFT", muteSoundButton, "RIGHT", 12, 0)

local comfortHint = panel:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
comfortHint:SetPoint("TOPLEFT", panel, "TOPLEFT", 18, -151)
comfortHint:SetWidth(334)
comfortHint:SetJustifyH("LEFT")
comfortHint:SetText("Masque seulement les refus de sorts du jeu. Les vraies erreurs Lua restent visibles.")

local function ApplyQuietMode()
    EnsureDatabase()
    if type(SetCVar) == "function" then
        pcall(SetCVar, "Sound_EnableErrorSpeech", CoAUIManagerDB.muteErrorSpeech and "0" or "1")
    end
    if UIErrorsFrame then
        if CoAUIManagerDB.hideErrorMessages and type(UIErrorsFrame.UnregisterEvent) == "function" then
            UIErrorsFrame:UnregisterEvent("UI_ERROR_MESSAGE")
        elseif not CoAUIManagerDB.hideErrorMessages and type(UIErrorsFrame.RegisterEvent) == "function" then
            UIErrorsFrame:RegisterEvent("UI_ERROR_MESSAGE")
        end
    end
end

local function UpdateQuietButtons()
    EnsureDatabase()
    muteSoundButton:SetText(CoAUIManagerDB.muteErrorSpeech and "SON D'ERREUR : OFF" or "SON D'ERREUR : ON")
    hideErrorsButton:SetText(CoAUIManagerDB.hideErrorMessages and "MESSAGES : MASQUÉS" or "MESSAGES : VISIBLES")
end

local function SetQuietMode(enabled)
    EnsureDatabase()
    CoAUIManagerDB.muteErrorSpeech = enabled and true or false
    CoAUIManagerDB.hideErrorMessages = enabled and true or false
    ApplyQuietMode()
    UpdateQuietButtons()
end

muteSoundButton:SetScript("OnClick", function()
    CoAUIManagerDB.muteErrorSpeech = not CoAUIManagerDB.muteErrorSpeech
    ApplyQuietMode()
    UpdateQuietButtons()
end)
hideErrorsButton:SetScript("OnClick", function()
    CoAUIManagerDB.hideErrorMessages = not CoAUIManagerDB.hideErrorMessages
    ApplyQuietMode()
    UpdateQuietButtons()
end)
...
local function BuildMinimapButton()
    ...
    minimapButton:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("UI Manager - Warmane", 1, 0.82, 0.20)
...
local function PrintHelp()
    ...
    Chat("/cui quiet on|off : masquer/rétablir le son et les messages d'échec de sorts")
end
...
SlashCmdList.COAUI = function(message)
    ...
    elseif command == "quiet" or command == "silence" then
        local action = string.lower(arguments or "")
        if action == "on" or action == "oui" then
            SetQuietMode(true)
            Chat("Son et messages d'échec de sorts masqués.")
        elseif action == "off" or action == "non" then
            SetQuietMode(false)
            Chat("Son et messages d'échec de sorts rétablis.")
        else
            Chat("Silence: son " .. (CoAUIManagerDB.muteErrorSpeech and "OFF" or "ON") .. ", messages " .. (CoAUIManagerDB.hideErrorMessages and "masqués" or "visibles") .. ".")
        end
